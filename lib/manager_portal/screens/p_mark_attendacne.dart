...
class ManagerPartTimeAttendanceScreen extends StatefulWidget {
  static String ID = "/manager_PartTime_attendance_screen";
  const ManagerPartTimeAttendanceScreen({super.key});

  @override
  State<ManagerPartTimeAttendanceScreen> createState() => _ManagerPartTimeAttendanceScreenState();
}

class _ManagerPartTimeAttendanceScreenState extends State<ManagerPartTimeAttendanceScreen> {
  ...
  void _initializeAttendanceState() {
    if (Constants.managerWorkingHours != null) {
      ...
      DateTime startBreakTime = _getDateTimeFromTimeString(Constants.managerWorkingHours!.startBreakTime);
      DateTime endBreakTime = _getDateTimeFromTimeString(Constants.managerWorkingHours!.endBreakTime);
      DateTime workEndTime = _getDateTimeFromTimeString(Constants.managerWorkingHours!.endTime);
  ...
  void _checkIn() async {
    ...
    if (_isPointInPolygon(locationData, companyCoordinates)) {
      await ManagerController
          .managerCheckIn(Constants.managerWorkingHours!.startPartTime!, '${DateTime.now().hour}:${DateTime.now().minute}', 'onjob')
          .then((value){
        setState(() {
          isCheckedIn = true;
          isInsideCompany = true;
