...
class ManagerFullTimeAttendanceScreen extends StatefulWidget {
  static String ID = "/manager_fullTime_attendance_screen";
  const ManagerFullTimeAttendanceScreen({super.key});

  @override
  State<ManagerFullTimeAttendanceScreen> createState() =>
      _ManagerFullTimeAttendanceScreenState();
}

class _ManagerFullTimeAttendanceScreenState
    extends State<ManagerFullTimeAttendanceScreen> {
  ...
  void _initializeAttendanceState() {
    ...
    if (Constants.managerWorkingHours != null) {
      ...
      DateTime startBreakTime = _getDateTimeFromTimeString(
          Constants.managerWorkingHours!.startBreakTime);
      DateTime endBreakTime = _getDateTimeFromTimeString(
          Constants.managerWorkingHours!.endBreakTime);
      DateTime workEndTime =
          _getDateTimeFromTimeString(Constants.managerWorkingHours!.endTime);
      if (Constants.attendanceDetail != null) {
        setState(() {
          isCheckedIn = true;
          _checkInTime = _getDateTimeFromTimeString(
              Constants.attendanceDetail!.checkingTime);
          print(Constants.attendanceDetail!.checkoutTime);
          if (Constants.attendanceDetail!.checkoutTime != null) {
            isCheckedOut = true;
            _checkOutTime = _getDateTimeFromTimeString(
                Constants.attendanceDetail!.checkoutTime);
          }
        });
      } else {
        setState(() {
  ...
  void _startBreakCheck() {
    if (Constants.managerWorkingHours != null) {
      ...
      DateTime workEndTime =
          _getDateTimeFromTimeString(Constants.managerWorkingHours!.endTime);
      print(startBreakTime.isAfter(now));
      print(workEndTime.isAfter(now));
      print(endBreakTime.isAfter(now));
  ...
  bool _isPointInPolygon(
      LocationData point, List<Map<String, double>> polygon) {
  ...
  void _checkIn() async {
    try {
      ...
      if (_isPointInPolygon(locationData, companyCoordinates)) {
        await ManagerController.managerCheckIn(
                Constants.managerWorkingHours!.startTime!,
        ...
        _showMessage(
            'Warning', 'You are not in office place right now.', false);
      }
    } catch (e) {
      print(e);
      setState(() {
      ...
      _showMessage('Sorry for Inconvenience', 'Try again later', false);
    }
  }
  ...
  _checkOut() async {
    try {
      ...
      LocationData locationData = await location.getLocation();
      if (_isPointInPolygon(locationData, companyCoordinates)) {
        await ManagerController.managerCheckOut(
                Constants.managerWorkingHours!.endTime!,
                '${DateTime.now().hour}:${DateTime.now().minute}',
                _calculateTotalHours(
                    _getDateTimeFromTimeString(
                        Constants.attendanceDetail!.checkingTime!),
                    DateTime.now()))
            .then((value) {
          setState(() {
      ...
      print(e);
      _showMessage('Sorry for inconvince', 'Try again later', false);
    }
  }
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            _buildAttendanceCard(
                'Absents', '05', Colors.blue[100]!, Colors.blue),
          ],
        ),
        SizedBox(height: 10),
        Row(
          ...
          children: [
            ...
            _buildAttendanceCard(
                'Leaves', '08', Colors.orange[100]!, Colors.orange),
          ],
        ),
      ],
    );
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Your break time will end at ${Constants.managerWorkingHours!.endBreakTime!}',
            textAlign: TextAlign.center,
            style: const TextStyle(
              fontSize: 13,
              color: Colors.grey,
