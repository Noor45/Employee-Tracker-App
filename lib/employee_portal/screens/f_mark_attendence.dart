...
class EmployeeFullTimeAttendanceScreen extends StatefulWidget {
  static String ID = "/employee_fullTime_attendance_screen";
  const EmployeeFullTimeAttendanceScreen({super.key});

  @override
  State<EmployeeFullTimeAttendanceScreen> createState() => _EmployeeFullTimeAttendanceScreenState();
}

class _EmployeeFullTimeAttendanceScreenState extends State<EmployeeFullTimeAttendanceScreen> {
    ...
    try {
      ...
      if (_isPointInPolygon(locationData, companyCoordinates)) {
        await EmployeeController
            .employeeCheckIn(Constants.employeeWorkingHours!.startTime!, '${DateTime.now().hour}:${DateTime.now().minute}', 'onjob')
            .then((value){
          setState(() {
            isCheckedIn = true;
            isInsideCompany = true;
